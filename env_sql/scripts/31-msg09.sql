...
CREATE
OR REPLACE FUNCTION public.stock_update() RETURNS trigger LANGUAGE 'plpgsql' NOT LEAKPROOF AS $ BODY $ begin -- new :: order_content, influence :: stock
update
    stock as s
set
    quantity = s.quantity + (
...
UPDATE
    OR
INSERT
    ON public.order_content FOR EACH ROW EXECUTE FUNCTION public.stock_update();